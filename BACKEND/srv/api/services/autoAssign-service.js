...
//  ENDPOINTS OFICIALES
// ============================
const USER_GETALL = `${dotenvXConfig.DOMINIO_USUARIOS}?ProcessType=getAll&DBServer=MongoDB&LoggedUser=AGUIZARE`;

const USER_GETBYID = `${dotenvXConfig.DOMINIO_USUARIOS}?ProcessType=getAll&DBServer=MongoDB&LoggedUser=AGUIZARE`;
...
export async function runAutoAssign() {
  try {
    ...
    const allUsers = resAll?.data?.value?.[0]?.data?.[0]?.dataRes || [];
    ...
    for (const u of allUsers) {
      ...
      const userReal = detail?.data?.value?.[0]?.data?.[0]?.dataRes;
